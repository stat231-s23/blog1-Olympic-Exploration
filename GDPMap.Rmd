...
medalperpop <- gdp_pop_2016 %>%
  ...
  summarise(
    MedalsPerPopulation = Total/Population,
    Country = Country
  )

map_data_gdp3 <- full_join(world_map, medalperpop, by = c("region" = "Country"))

saveRDS(map_data_gdp, file = "map_data_gdp3.rds")

mapplot5 <- ggplot(map_data_gdp3, aes(x = long, y = lat, group = group, fill = MedalsPerPopulation, text = region)) +
  geom_polygon(color = "black") +
  scale_fill_viridis(option = "magma", direction = -1) + 
